...
class DecodeDetections(Layer):
    ...
    def world_coordinates_to_streetview_pixel(self, lat, lng, lat1,lng1, yaw, image_width, image_height,height=0, zoom=None, object_dims=None, method=None):
        ...
        pitch = 0
        dx1 = math.cos(math.radians(lat1))
        dx11 = lng-lng1
        dxr = tf.sin(dx11 * (math.pi/180))
        dx = dx1 * dxr

        dy11 = lat-lat1
        dyr = dy11 * (math.pi/180)
        dy = tf.sin(dyr)

        look_at_angle = math.pi + tf.atan2(dx, dy) - yaw  
        i = tf.constant(np.float64(2*math.pi))
        
        c = lambda x : tf.reduce_any(tf.greater(x, i))
        b = lambda x : tf.subtract(x, tf.cast(tf.greater(x, i), tf.float64)*2*math.pi)
        look_at_angle = tf.while_loop(c, b, [look_at_angle])

        t = lambda x : tf.reduce_any(tf.less(x, 0))
        d = lambda x : tf.add(x, tf.cast(tf.less(x, 0), tf.float64)*2*math.pi) 
        look_at_angle = tf.while_loop(t, d, [look_at_angle])

        z = tf.sqrt(dx*dx+dy*dy)*EARTH_RADIUS
        x = (image_width*look_at_angle)/(2*math.pi)
        camhei = tf.constant(np.float64(height-camera_height))
        y = image_height/2 - image_height*(tf.atan2(camhei, z)-pitch)/(math.pi) 
        return x, y 

    def streetview_pixel_to_world_coordinates(self, lat1,lng1, yaw, image_width, image_height, x, y):
        camera_height = float(GOOGLE_CAR_CAMERA_HEIGHT)  # ballpark estimate of the number of meters that camera is off the ground
        pitch = float(0)
        look_at_angle = x * (2*math.pi)/float(image_width)
        height = 0
        tilt_angle = (image_height/2-y)*math.pi/image_height+pitch
        tilt_angle = tf.cast(tilt_angle, tf.float64)

        z_ = K.minimum(np.float64(-1e-2),tilt_angle)
        z = (height-camera_height) / tf.tan(z_)

        dx = tf.sin(look_at_angle-math.pi+yaw)*z/EARTH_RADIUS
        dy = tf.cos(look_at_angle-math.pi+yaw)*z/EARTH_RADIUS    
        
        lat = lat1 + tf.asin(dy) * 180/math.pi
        lng = lng1 + tf.asin(dx/math.cos((lat1)*math.pi/180)) * 180/math.pi
        return lat,lng
    ...
    def call(self, y_pred, mask=None):
        ...
        lat, lng = streetview_pixel_to_world_coordinates(34.142629,-118.135047,4.737696254538608, 600, 300, x, y)
        x_, y_ = world_coordinates_to_streetview_pixel(lat, lng, 34.142619,-118.134846, 4.781154952913266, 600, 300)
        ...
        # Concatenate the one-hot class confidences and the converted box coordinates to form the decoded predictions tensor.
        y_pred = tf.concat(values=[xmin_, ymin_, xmax_, ymax_], axis=-1)
