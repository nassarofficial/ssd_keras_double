...
class SSDLoss_proj:
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        def gt_rem(pred, gt):
            ...
            gt = tf.slice(gt, [0, 0, 0], [1, tf.shape(pred)[0], 15],name="rem_slice")
            return gt
        ...
        def gt_add(pred, gt):
            ...
            ext = tf.slice(gt, [0, 0, 0], [1, 1, 15], name="add_slice")
            multiply = [1,val,1]
        ...
        def matcher(y_true_1,y_pred_1,y_true_2,y_pred_2, bsz):
            ...
            for i in range(bsz):
                ...
                set1 = tf.cast(y_true_new[:,:,-1],dtype=tf.int32)
                set2 = tf.cast(y_true_2_new[:,:,-1],dtype=tf.int32)

                id_pick = tf.sets.set_intersection(set1[None,:], set2[None, :])
                id_pick = tf.cast(id_pick.values[0],dtype=tf.float32)
                            
                filterer = tf.where(tf.equal(y_true_1[i,:,-1],id_pick))
                filterer_2 = tf.where(tf.equal(y_true_2[i,:,-1],id_pick))
                ...
                y_pred_1_new = y_pred_1[i,:,-13:-9]
                # iou_out = tf.py_func(iou_float, [y_pred_1_new,y_true_new[0,:,-16:-12]], tf.float32, name="iou_out")
                iou_out = tf_iou(y_pred_1_new,y_true_new[0,:,-13:-9])
                # bipartite_matches = tf.py_func(match_bipartite_greedy32, [iou_out], tf.int32, name="bipartite_matches")
                rows = tf.shape(iou_out)[0]
                rows = tf.cast(rows, dtype=tf.float32)

                bipartite_matches,bipartite_matches1 = tf.contrib.image.bipartite_match(iou_out,rows)

                # bipartite_matches = match_bipartite_greedy_tf(iou_out)
        ...
        classification_loss = tf.to_float(self.log_loss(y_true_out[:,:,:-13], y_pred_out[:,:,:-13]), name="classification") # Output shape: (batch_size, n_boxes)
        localization_loss = tf.to_float(self.smooth_L1_loss(y_true_out[:,:,-13:-9], y_pred_out[:,:,-13:-9]),name="localization_loss") # Output shape: (batch_size, n_boxes)
        ...
        positives = tf.to_float(tf.reduce_max(y_true_out[:,:,1:-13], axis=-1),name="positives") # Tensor of shape (batch_size, n_boxes)
