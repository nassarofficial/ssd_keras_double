...
#         self.n_neg_min = n_neg_min
#         self.alpha = alpha
#         self.EARTH_RADIUS = tf.cast(6371000, tf.float32)  # Radius in meters of Earth
#         self.GOOGLE_CAR_CAMERA_HEIGHT = 3 # ballpark estimate of the number of meters that camera is off the ground
...
#     def loss_detection(self, y_true, y_pred):
#         batch_size = tf.shape(y_pred)[0] # Output dtype: tf.int32
#         n_boxes = tf.shape(y_pred)[1] # Output dtype: tf.int32, note that `n_boxes` in this context denotes the total number of boxes per image, not the number of boxes per cell.
...
#     def compute_loss_1(self, y_true, y_pred):
#         '''
#         Compute the loss of the SSD model prediction against the ground truth.
...
#         y_pred_1 = y_pred[:,:,:14]
#         # y_pred_1_proj = y_pred[:,:,14:28]
#         # y_pred_2 = y_pred[:,:,28:42]
#         # y_pred_2_proj = y_pred[:,:,42:]

#         y_true_1 = y_true[:,:,:14]
#         # y_true_2 = y_true[:,:,17:31]


#         # 1: Compute the losses for class and box predictions for every box.
#         loss_1 = self.loss_detection(y_true_1, y_pred_1)
#         # loss_2 = self.loss_detection(y_true_2, y_pred_2)

#         # # 1: Compute the losses for class and box predictions for every box projected to the other view.
#         # loss_1_2 = self.loss_detection(y_true_1, y_pred_1_proj)
#         # loss_2_1 = self.loss_detection(y_true_2, y_pred_2_proj)

#         return loss_1
#         # return (loss_1 + loss_2 + loss_1_2 + loss_2_1) / 4