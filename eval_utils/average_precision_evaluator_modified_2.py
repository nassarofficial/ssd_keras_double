...
class Evaluator:
    ...
    def __init__(self,
        ...
        self.prediction_results_1 = None
        self.prediction_results_2 = None
        self.prediction_results_1_on_2 = None
        self.prediction_results_2_on_1 = None
        self.num_gt_per_class_1 = None
        self.num_gt_per_class_2 = None
    ...
    def predict_on_dataset(self,
        ...
        for j in tr:
            ...
            y_pred_filtered1 = []
            y_pred_filtered2 = []
            y_pred_filtered1_on_2 = []
            y_pred_filtered2_on_1 = []

            for i in range(len(y_pred_1)):
                y_pred_filtered1.append(y_pred_1[i][y_pred_1[i,:,0] != 0])
            for i in range(len(y_pred_2)):
                y_pred_filtered2.append(y_pred_2[i][y_pred_2[i,:,0] != 0])
            for i in range(len(y_pred_1_on_2)):
                y_pred_filtered1_on_2.append(y_pred_1_on_2[i][y_pred_1_on_2[i,:,0] != 0])
            for i in range(len(y_pred_2_on_1)):
                y_pred_filtered2_on_1.append(y_pred_2_on_1[i][y_pred_2_on_1[i,:,0] != 0])

            y_pred_1 = y_pred_filtered1
            y_pred_1_on_2 = y_pred_filtered1_on_2
            y_pred_2 = y_pred_filtered2
            y_pred_2_on_1 = y_pred_filtered2_on_1

            # Convert the predicted box coordinates for the original images.
            y_pred_1 = apply_inverse_transforms(y_pred_1, batch_inverse_transforms)
            y_pred_1_on_2 = apply_inverse_transforms(y_pred_1_on_2, batch_inverse_transforms1)
            y_pred_2 = apply_inverse_transforms(y_pred_2, batch_inverse_transforms)
            y_pred_2_on_1 = apply_inverse_transforms(y_pred_2_on_1, batch_inverse_transforms1)

            # Iterate over all batch items.
            for k, batch_item in enumerate(y_pred_1):
                ...
                for box in batch_item:
                    ...
                    results_1[class_id].append(prediction)

            for k, batch_item in enumerate(y_pred_1_on_2):
                ...
                for box in batch_item:
                    ...
                    prediction = (image_id, confidence, xmin, ymin, xmax, ymax)
                    # Append the predicted box to the results list for its class.
                    results_1_on_2[class_id].append(prediction)

            for k, batch_item in enumerate(y_pred_2):
                image_id = batch_image_ids[k][1]
                ...
                for box in batch_item:
                    ...
                    prediction = (image_id, confidence, xmin, ymin, xmax, ymax)
                    # Append the predicted box to the results list for its class.
                    results_2[class_id].append(prediction)

            for k, batch_item in enumerate(y_pred_2_on_1):
                image_id = batch_image_ids[k][0]
                ...
                for box in batch_item:
                    ...
                    prediction = (image_id, confidence, xmin, ymin, xmax, ymax)
                    # Append the predicted box to the results list for its class.
                    results_2_on_1[class_id].append(prediction)


        self.prediction_results_1 = results_1
        self.prediction_results_1_on_2 = results_1_on_2
        self.prediction_results_2 = results_2
        self.prediction_results_2_on_1 = results_2_on_1

        if ret:
            return results_1, results_1_on_2, results_2, results_2_on_1
    ...
    def get_num_gt_per_class(self,
        ...
        num_gt_per_class_1 = np.zeros(shape=(self.n_classes+1), dtype=np.int)
        num_gt_per_class_2 = np.zeros(shape=(self.n_classes+1), dtype=np.int)

        class_id_index = self.gt_format['class_id']

        ground_truth_1 = self.data_generator.labels
        ground_truth_2 = self.data_generator.labels1
        ...
        for i in tr:
            ...
            boxes_gt_1 = np.asarray(ground_truth_1[i],dtype=np.int64)
            boxes_gt_2 = np.asarray(ground_truth_2[i],dtype=np.int64)
            boxes_1 = boxes_gt_1[:,:5]
            boxes_2 = boxes_gt_2[:,:5]
            ...
            for j in range(boxes_1.shape[0]):
                ...
                if ignore_neutral_boxes and not (self.data_generator.eval_neutral is None):
                    ...
                    if not self.data_generator.eval_neutral[i][j]:
                        ...
                        class_id = boxes_1[j, class_id_index]
                        num_gt_per_class_1[class_id] += 1
                else:
            ...
            for j in range(boxes_2.shape[0]):
                ...
                if ignore_neutral_boxes and not (self.data_generator.eval_neutral1 is None):
                    ...
                    if not self.data_generator.eval_neutral1[i][j]:
                        ...
                        # increment the counter for the respective class ID.
                        class_id = boxes[j, class_id_index]
                        num_gt_per_class_2[class_id] += 1
                else:
                    # If there is no such thing as evaluation-neutral boxes for
                    # our dataset, always increment the counter for the respective
                    # class ID.
                    # print("class_id: ",class_id)
                    class_id = boxes_2[j, class_id_index]
                    # print("num_gt_per_class: ",num_gt_per_class)
                    num_gt_per_class_2[class_id] += 1

        self.num_gt_per_class_1 = num_gt_per_class_1
        self.num_gt_per_class_2 = num_gt_per_class_2

        if ret:
            return num_gt_per_class_1, num_gt_per_class_2
    ...
    def match_predictions(self,
        ...
        eval_neutral_available1 = not (self.data_generator.eval_neutral1 is None) # Whether or not we have annotations to decide whether ground truth boxes should be neutral or not.
        ...
        for i in range(len(self.data_generator.image_ids1)):
        ...
        true_positives_1 = [[]] # The false positives for each class, sorted by descending confidence.
        false_positives_1 = [[]] # The true positives for each class, sorted by descending confidence.
        cumulative_true_positives_1 = [[]]
        cumulative_false_positives_1 = [[]]

        true_positives_2 = [[]] # The false positives for each class, sorted by descending confidence.
        false_positives_2 = [[]] # The true positives for each class, sorted by descending confidence.
        cumulative_true_positives_2 = [[]]
        cumulative_false_positives_2 = [[]]

        true_positives_1_on_2 = [[]] # The false positives for each class, sorted by descending confidence.
        false_positives_1_on_2  = [[]] # The true positives for each class, sorted by descending confidence.
        cumulative_true_positives_1_on_2  = [[]]
        cumulative_false_positives_1_on_2  = [[]]

        true_positives_2_on_1 = [[]] # The false positives for each class, sorted by descending confidence.
        false_positives_2_on_1 = [[]] # The true positives for each class, sorted by descending confidence.
        cumulative_true_positives_2_on_1 = [[]]
        cumulative_false_positives_2_on_1 = [[]]

        # Iterate over all classes.
        for class_id in range(1, self.n_classes + 1):

            predictions_1 = self.prediction_results_1[class_id]
            predictions_2 = self.prediction_results_2[class_id]
            predictions_1_on_2 = self.prediction_results_1_on_2[class_id]
            predictions_2_on_1 = self.prediction_results_2_on_1[class_id]

            # Store the matching results in these lists:
            true_pos_1 = np.zeros(len(predictions_1), dtype=np.int) # 1 for every prediction that is a true positive, 0 otherwise
            false_pos_1 = np.zeros(len(predictions_1), dtype=np.int) # 1 for every prediction that is a false positive, 0 otherwise

            true_pos_2 = np.zeros(len(predictions_2), dtype=np.int) # 1 for every prediction that is a true positive, 0 otherwise
            false_pos_2 = np.zeros(len(predictions_2), dtype=np.int) # 1 for every prediction that is a false positive, 0 otherwise

            true_pos_1_on_2 = np.zeros(len(predictions_1_on_2), dtype=np.int) # 1 for every prediction that is a true positive, 0 otherwise
            false_pos_1_on_2 = np.zeros(len(predictions_1_on_2), dtype=np.int) # 1 for every prediction that is a false positive, 0 otherwise

            true_pos_2_on_1 = np.zeros(len(predictions_2_on_1), dtype=np.int) # 1 for every prediction that is a true positive, 0 otherwise
            false_pos_2_on_1 = np.zeros(len(predictions_2_on_1), dtype=np.int) # 1 for every prediction that is a false positive, 0 otherwise
            ...
            if len(predictions_1) == 0:
                ...
                true_positives_1.append(true_pos_1)
                false_positives_1.append(false_pos_1)
                continue

            if len(predictions_2) == 0:
                print("No predictions for class {}/{}".format(class_id, self.n_classes))
                true_positives_2.append(true_pos_2)
                false_positives_2.append(false_pos_2)
                continue

            if len(predictions_1_on_2) == 0:
                print("No predictions for class {}/{}".format(class_id, self.n_classes))
                true_positives_1_on_2.append(true_pos_1_on_2)
                false_positives_1_on_2.append(false_pos_1_on_2)
                continue

            if len(predictions_2_on_1) == 0:
                print("No predictions for class {}/{}".format(class_id, self.n_classes))
                true_positives_2_on_1.append(true_pos_2_on_1)
                false_positives_2_on_1.append(false_pos_2_on_1)
                continue
            ...
            num_chars_per_image_id_1 = len(str(predictions_1[0][0])) + 6 # Keep a few characters buffer in case some image IDs are longer than others.
            num_chars_per_image_id_2 = len(str(predictions_2[0][0])) + 6 # Keep a few characters buffer in case some image IDs are longer than others.
            num_chars_per_image_id_1_on_2 = len(str(predictions_1_on_2[0][0])) + 6 # Keep a few characters buffer in case some image IDs are longer than others.
            num_chars_per_image_id_2_on_1 = len(str(predictions_2_on_1[0][0])) + 6 # Keep a few characters buffer in case some image IDs are longer than others.

            # Create the data type for the structured array.
            preds_data_type_1 = np.dtype([('image_id', 'U{}'.format(num_chars_per_image_id_1)),
                                        ('confidence', 'f4'),
                                        ('xmin', 'f4'),
                                        ('ymin', 'f4'),
                                        ('xmax', 'f4'),
            ...
            preds_data_type_2 = np.dtype([('image_id', 'U{}'.format(num_chars_per_image_id_2)),
                                        ('confidence', 'f4'),
                                        ('xmin', 'f4'),
                                        ('ymin', 'f4'),
                                        ('xmax', 'f4'),
            ...
            preds_data_type_1_on_2 = np.dtype([('image_id', 'U{}'.format(num_chars_per_image_id_1_on_2)),
                                        ('confidence', 'f4'),
                                        ('xmin', 'f4'),
                                        ('ymin', 'f4'),
                                        ('xmax', 'f4'),
            ...
            preds_data_type_2_on_1 = np.dtype([('image_id', 'U{}'.format(num_chars_per_image_id_2_on_1)),
                                        ('confidence', 'f4'),
                                        ('xmin', 'f4'),
                                        ('ymin', 'f4'),
                                        ('xmax', 'f4'),
            ...
            predictions_1 = np.array(predictions_1, dtype=preds_data_type_1)
            predictions_2 = np.array(predictions_2, dtype=preds_data_type_2)
            predictions_1_on_2 = np.array(predictions_1_on_2, dtype=preds_data_type_1_on_2)
            predictions_2_on_1 = np.array(predictions_2_on_1, dtype=preds_data_type_2_on_1)
