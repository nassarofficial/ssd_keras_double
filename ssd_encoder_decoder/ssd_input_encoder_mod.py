...
class SSDInputEncoder:
    ...
    def __call__(self, ground_truth_labels, diagnostics=False):
        ...
        for i in range(batch_size): # For each batch item...
            ...
            if self.matching_type == 'multi':
                ...
                # print("y_encoded[i, matches[1], :-12]: ", y_encoded[i, matches[1], :-12])
                # print("y_encoded[i, matches[1], -4:]: ", y_encoded[i, matches[1], -4:])
                # print("y_encoded: ", y_encoded[i, matches[1], :])
    ...
    def generate_encoding_template(self, batch_size, diagnostics=False):
        ...
        lat_tensor = np.ones((batch_size, boxes_tensor.shape[1], 1)) * 199
        lng_tensor = np.ones((batch_size, boxes_tensor.shape[1], 1)) * 199
